...
-- 0.1 SECOND BLINKING SPEED!
local flickerSpeed = 0.1          -- 0.1 second show, 0.1 second hide (PERFECT BLINK!)
local colorChangeSpeed = 0.08
...
local function findRemotes()
    ...
    local bioNames = {"UpdateBio", "SetBio", "ChangeBio"}
    ...
    bioRemote = search(ReplicatedStorage, bioNames)
end
...
-- BLINKING LOOP (0.1 second show, 0.1 second hide)
...
local function startAll()
    ...
    if bioThread and coroutine.status(bioThread) ~= "dead" then
...
phraseTitleText.Text = "📝 ADD PHRASES"
...
instrLabel.Text = "Separate with commas"
...
print("👁️ BLINKING BIO ACTIVE!")
print("📝 0.1s Show → 0.1s Hide → Next phrase (Perfect Blink)")
